"""A knob-push notice briefly overrides the now-playing rows."""
from km_stereo import Readout


def test_a_notice_overrides_the_readout_for_a_moment_then_the_player_returns():
    ...
    assert state.tune({'title': 'jazz lofi radio beats to chill', 'line': 'STOPPING', 'hold': 3}, 500)
    assert state.frame(600) == ('JAZZ LOFI RADIO', 'STOPPING')
    assert state.frame(3499) == ('JAZZ LOFI RADIO', 'STOPPING')
    assert state.showing_notice(3499)
    state.receive({'title': 'synth ambient radio', 'artist': 'Lofi Girl'}, 3000)
    assert state.frame(3500) == ('SYNTH AMBIENT RADIO', 'LOFI GIRL')
    assert not state.showing_notice(3500)


def test_a_notice_rejects_junk_and_shows_only_the_first_page_of_long_names():
    state = Readout(lambda a, b: a - b)
    assert not state.tune({'title': 'x' * 97, 'line': 'LOADING', 'hold': 3}, 0)
    assert not state.tune({'title': 'ok', 'line': 5, 'hold': 3}, 0)
    assert not state.tune({'title': 'ok', 'line': 'T', 'hold': 'no'}, 0)
    assert state.tune({'title': 'lofi hip hop radio beats to relax/study to', 'line': 'LOADING', 'hold': 3}, 0)
    assert state.frame(0) == ('LOFI HIP HOP RADIO', 'LOADING')
    assert state.frame(2999) == ('LOFI HIP HOP RADIO', 'LOADING')   # a notice never pages
...
def test_a_new_title_from_the_player_cancels_a_pending_tune_line():
    ...
    assert not state.showing_notice(4000)
